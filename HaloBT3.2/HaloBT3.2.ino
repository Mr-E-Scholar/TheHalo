...
THE HALO LIGHT CONTROLLER BASIC VERSION 3.2 (FOR TEENSY 3.2)
...
void loop() {
 ...
 if (tog == HIGH) { 
    ...
    float B = ((DIAL0 * .24)+6); ring.setBrightness(B); testLED.setPixelColor(0, 0); doAction(); state=LOW;}
 else if (tog == LOW) { 
...
void doAction() { 
  ...
  if (membut == LOW) { 
    delay(800); Enc.write(0);
    if (OPTM == true) { startAddress = 0; }
    else if (OPTM == false) { startAddress = 312; }
    writeToEEPROM(startAddress, pixelsL, 312); OPTM=!OPTM;}  
    delay(30);
if (digitalRead(Btn)==LOW) {delay(100); encodeL=true; encodecountL=!encodecountL;}
...
if (digitalRead(Btn)==LOW) {delay(100); encode=true; encodecount=!encodecount; 
  if (PIX<104) { ring.setPixelColor(PIX, pixelsM[PIX], pixelsM[3*PIX+1], pixelsM[3*PIX+2]);}
  else if (104<=PIX) { ring.setPixelColor(PIX, 0);}
  if (memcnt == false) {writeToEEPROM(startAddress, pixelsM, 312);}
  memcnt=!memcnt;
}
...
if (digitalRead(Rfour)==LOW) { pixelsM[pl]=CRN; pixelsM[pl+1]=CGN; pixelsM[pl+2]=CBN;}
  ring.setPixelColor(lastpix, pixelsM[pl], pixelsM[pl+1], pixelsM[pl+2]); ring.show(); lastpix=PIX;   
}
for (int j=0; j<TOT; j++) { int jj=3*j; ring.setPixelColor(j, pixelsM[jj], pixelsM[jj+1], pixelsM[jj+2]);}; delay(30); ring.setPixelColor(PIX, abs(255-pixelsM[3*PIX]), abs(255-pixelsM[3*PIX+1]), abs(255-pixelsM[3*PIX+2])); ring.show();
}
...
void doAlterArrayLive() { delay(30);
...
if (PosL != count ) { //delay(2);
  valRotary = (((PosL % endnum)+endnum) % endnum); count = PosL; PIX = abs(startnum + valRotary); p=3*PIX; int pl=3*lastpix;
  int DIAL1 = analogRead(RED); int DIAL2 = analogRead(GREEN); int DIAL3 = analogRead(BLUE);
  float CRN = (DIAL1*.24); float CGN = (DIAL2*.24); float CBN = (DIAL3*.24); testLED.setPixelColor(0, CRN, CGN, CBN); 
if (digitalRead(Rfour)==LOW) { pixelsL[pl]=CRN; pixelsL[pl+1]=CGN; pixelsL[pl+2]=CBN;}
  ring.setPixelColor(lastpix, pixelsL[pl], pixelsL[pl+1], pixelsL[pl+2]); ring.show(); lastpix=PIX;   
}
  

for (int j=0; j<TOT; j++) { int jj=3*j; ring.setPixelColor(j, pixelsL[jj], pixelsL[jj+1], pixelsL[jj+2]);} delay(30); ring.setPixelColor(PIX, abs(255-pixelsL[3*PIX]), abs(255-pixelsL[3*PIX+1]), abs(255-pixelsL[3*PIX+2])); ring.show();
for (int i=0; i<312; i++) {pixelsM[i]=pixelsL[i];}
}
